public class Calculator {
	...
	/**
	 * Checks if inputs are valid dates 01/01/1901 and 31/12/2999.
	 * @param  d1 the start date
	 * @param  d2 the end date
	 * @return boolean true if date is valid
	 */	
	public static boolean checkValidDate(MyDate d1, MyDate d2){
		if(d1.getYear() > 1900 && d2.getYear() < 3000){
			return true;
		}
		return false;
	}
	...
	public static int dayswithinMonths(MyDate d1, MyDate d2){
		...
		if(d1.getMonth() == d2.getMonth() && d1.getYear() == d2.getYear()){
			...
			return d1.getallDaysinMonth(d1.getMonth()) - d1.getDay() + d2.getDay();
		}
	}
	...
	public static void main(String[] args) {
		...
		try{
			...
			if(!checkValidDate(date1, date2)){
				System.out.println("Please enter valid dates within: 01/01/1901 - 31/12/2999.");
				return;
			}
